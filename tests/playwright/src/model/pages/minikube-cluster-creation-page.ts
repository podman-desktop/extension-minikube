    ...
    return test.step('Create parametrized cluster', async () => {
      ...
      if (driver) {
        try{ 
          ...
          const driverOptions = this.clusterPropertiesInformation.getByLabel('Driver Options'); 
          await playExpect(driverOptions).toBeVisible();
          const driverButton = driverOptions.getByRole('button', { name: driver, exact: true });
      ...
      if (containerRuntime) {
        try{ 
          ...
          const containerRuntimeOptions = this.clusterPropertiesInformation.getByLabel('Container Runtime Options');
          await playExpect(containerRuntimeOptions).toBeVisible();
          const containerRuntimeButton = containerRuntimeOptions.getByRole('button', { name: containerRuntime, exact: true });
